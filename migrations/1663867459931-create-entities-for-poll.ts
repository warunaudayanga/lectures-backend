...
export class createEntitiesForPoll1663867459931 implements MigrationInterface {
    name = 'createEntitiesForPoll1663867459931'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`poll_vote\` (\`id\` int NOT NULL AUTO_INCREMENT, \`option\` json NOT NULL, \`status\` enum ('ACTIVE', 'INACTIVE') NOT NULL DEFAULT 'ACTIVE', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`pollId\` int NOT NULL, \`createdById\` int NULL, \`updatedById\` int NULL, \`deletedById\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`polls\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`code\` varchar(255) NOT NULL, \`description\` varchar(255) NULL, \`endTime\` datetime NULL, \`startTime\` datetime NULL, \`options\` json NOT NULL, \`requireIdentity\` tinyint NOT NULL DEFAULT 0, \`removable\` tinyint NOT NULL DEFAULT 0, \`updatable\` tinyint NOT NULL DEFAULT 0, \`status\` enum ('ACTIVE', 'INACTIVE') NOT NULL DEFAULT 'ACTIVE', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`createdById\` int NULL, \`updatedById\` int NULL, \`deletedById\` int NULL, UNIQUE INDEX \`CODE\` (\`code\`), UNIQUE INDEX \`NAME\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`poll_voters\` (\`pollId\` int NOT NULL, \`userId\` int NOT NULL, INDEX \`IDX_224f39616e4c1b63ff2177614c\` (\`pollId\`), INDEX \`IDX_20886b39c3b5c2fd3dee38d1a6\` (\`userId\`), PRIMARY KEY (\`pollId\`, \`userId\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`poll_voters\` ADD CONSTRAINT \`FK_224f39616e4c1b63ff2177614c5\` FOREIGN KEY (\`pollId\`) REFERENCES \`polls\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`poll_voters\` ADD CONSTRAINT \`FK_20886b39c3b5c2fd3dee38d1a60\` FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
